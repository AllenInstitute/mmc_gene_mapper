"""
Define a more systematic set of test data
"""
import pytest
...
def ncbi_file_package_fixture(
    ...
    Genes will have symbols like 'symbol_{ii}'
    where '{ii}' is the mouse integer index

    ENSEMBL genes will have names like
    e_{species}_{ii}

    human will have an extra gene
    ENSG8888 with symbol symbol_8888
    for unit testing purposes (to break multi-species degeneracy)

    mouse and fish will have orthologs on all even genes
    fish and humans will have orthologs on all multiples of 3
    ("even" and "multiples of 3" reckoned on mouse ID)
    ...
    for ii in range(n_genes):
        ...
        mouse_bkbit.append(
            {"category": ["bican:GeneAnnotation"],
             ...
             "symbol": f"symbol_{mouse_ncbi['GeneID']}",
             "name": f"e_mouse_{mouse_ncbi['GeneID']}",
        ...
        if ii % 2 == 0:
            ...
            human_bkbit.append(
                {"category": ["bican:GeneAnnotation"],
                 ...
                 "symbol": f"symbol_{mouse_ncbi['GeneID']}",
                 "name": f"e_human_{mouse_ncbi['GeneID']}",
            ...
            fish_bkbit.append(
                {"category": ["bican:GeneAnnotation"],
                 ...
                 "symbol": f"symbol_{mouse_ncbi['GeneID']}",
                 "name": f"e_fish_{mouse_ncbi['GeneID']}",
                 "in_taxon_label": "pisces"}
            )

        if mouse_ncbi['GeneID'] % 2 == 0:
